...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Linux | x86_64 | OpenJDK 25.0.1 | **FAIL** (headless probe exit `2`: OpenGL backend instantiation failed with `HeadlessException`) | **FAIL** (GUI acceptance scenario blocked at startup; no display server in this run) | **FAIL** (camera/picking checks cannot execute because startup fails before viewer creation) | **FAIL** (offscreen OpenGL path unavailable; startup/backend probe failed) | **FAIL** | **YES** | `java -Dorg.gavrog.3dt.opengl.backends=de.jreality.jogl.Viewer -Djava.library.path=Deploy/hardware/linux-x86_64 -cp /tmp/probeclasses:Deploy/jReality/jReality.jar:Deploy/jogl/jogl.jar:Deploy/jogl/gluegen-rt.jar org.gavrog.apps._3dt.RendererProbe` → exit `2` |
| Windows | x86_64 | OpenJDK 25.0.1 (Linux-hosted JVM with `-Dos.name`/`-Dos.arch` override for bundle targeting) | **FAIL** (probe exit `2`: OpenGL backend instantiation failed with `HeadlessException`) | **FAIL** (GUI acceptance scenario blocked at startup; no display server in this run) | **FAIL** (camera/picking checks cannot execute because startup fails before viewer creation) | **FAIL** (offscreen OpenGL path unavailable; startup/backend probe failed) | **FAIL** | **YES** | `java -Dos.name="Windows 10" -Dos.arch=amd64 -Dorg.gavrog.3dt.opengl.backends=de.jreality.jogl.Viewer -Djava.library.path=Deploy/hardware/windows-x86_64 -cp /tmp/probeclasses:Deploy/jReality/jReality.jar:Deploy/jogl/jogl.jar:Deploy/jogl/gluegen-rt.jar org.gavrog.apps._3dt.RendererProbe` → exit `2`; `java -Dos.name="Windows 10" -Dos.arch=amd64 -Dorg.gavrog.3dt.opengl.backends=de.jreality.jogl.Viewer -Djava.library.path=Deploy/hardware/windows-x86_64 -cp /tmp/probeclasses:Deploy/jReality/jReality.jar:Deploy/jogl/jogl.jar:Deploy/jogl/gluegen-rt.jar org.gavrog.apps._3dt.RendererSelfCheck` → native loads all `[FAIL]` (`no ... in java.library.path`) |
| macOS | x86_64 | OpenJDK 25.0.1 (Linux-hosted JVM with `-Dos.name`/`-Dos.arch` override for bundle targeting) | **FAIL** (probe exit `2`: OpenGL backend instantiation failed with `HeadlessException`) | **FAIL** (GUI acceptance scenario blocked at startup; no display server in this run) | **FAIL** (camera/picking checks cannot execute because startup fails before viewer creation) | **FAIL** (offscreen OpenGL path unavailable; startup/backend probe failed) | **FAIL** | **YES** | `java -Dos.name="Mac OS X" -Dos.arch=x86_64 -Dorg.gavrog.3dt.opengl.backends=de.jreality.jogl.Viewer -Djava.library.path=Deploy/hardware/macos-x86_64 -cp /tmp/probeclasses:Deploy/jReality/jReality.jar:Deploy/jogl/jogl.jar:Deploy/jogl/gluegen-rt.jar org.gavrog.apps._3dt.RendererProbe` → exit `2`; `java -Dos.name="Mac OS X" -Dos.arch=x86_64 -Dorg.gavrog.3dt.opengl.backends=de.jreality.jogl.Viewer -Djava.library.path=Deploy/hardware/macos-x86_64 -cp /tmp/probeclasses:Deploy/jReality/jReality.jar:Deploy/jogl/jogl.jar:Deploy/jogl/gluegen-rt.jar org.gavrog.apps._3dt.RendererSelfCheck` → native loads all `[FAIL]` (`no ... in java.library.path`) |
| macOS | arm64 | OpenJDK 25.0.1 (Linux-hosted JVM with `-Dos.name`/`-Dos.arch` override for bundle targeting) | **FAIL** (probe exit `2`: OpenGL backend instantiation failed with `HeadlessException`) | **FAIL** (GUI acceptance scenario blocked at startup; no display server in this run) | **FAIL** (camera/picking checks cannot execute because startup fails before viewer creation) | **FAIL** (offscreen OpenGL path unavailable; startup/backend probe failed) | **FAIL** | **YES** | `java -Dos.name="Mac OS X" -Dos.arch=aarch64 -Dorg.gavrog.3dt.opengl.backends=de.jreality.jogl.Viewer -Djava.library.path=Deploy/hardware/macos-arm64 -cp /tmp/probeclasses:Deploy/jReality/jReality.jar:Deploy/jogl/jogl.jar:Deploy/jogl/gluegen-rt.jar org.gavrog.apps._3dt.RendererProbe` → exit `2`; `java -Dos.name="Mac OS X" -Dos.arch=aarch64 -Dorg.gavrog.3dt.opengl.backends=de.jreality.jogl.Viewer -Djava.library.path=Deploy/hardware/macos-arm64 -cp /tmp/probeclasses:Deploy/jReality/jReality.jar:Deploy/jogl/jogl.jar:Deploy/jogl/gluegen-rt.jar org.gavrog.apps._3dt.RendererSelfCheck` → native loads all `[FAIL]` (`no ... in java.library.path`) |
...
For all three requested platform-native OpenGL rows (Windows x86_64, macOS
x86_64, macOS arm64), this execution produced explicit **FAIL** outcomes with
exact probe/self-check evidence commands. The release gate therefore remains
blocked, and no row may be marked green until the same checks pass in a true
GUI-capable native runtime with populated platform-native bundles.
